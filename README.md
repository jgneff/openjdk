...
OpenJDK Runtime Environment (build 26.0.2.1+1-snap)
OpenJDK 64-Bit Server VM (build 26.0.2.1+1-snap, mixed mode, sharing)
```
...
| 2026-06-04 | Rampdown Phase One        | 26 | ←  | 27 | 28 |
| 2026-07-16 | Rampdown Phase Two        | 26 | ←  | 27 | 28 |
| 2026-08-06 | Initial Release Candidate | 26 | 27 | ←  | 28 |
| 2026-08-20 | Final Release Candidate   | 26 | 27 | ←  | 28 |
| 2026-09-15 | General Availability      | 27 | ←  | ←  | 28 |
...
| ------------ | ------------------- | ------------------- | ---------------------- |
| candidate    | [snapcraft.yaml][1] | [openjdk/jdk26u][4] | [openjdk-candidate][7] |
| beta         | [snapcraft.yaml][2] | [openjdk/jdk27u][5] | [openjdk-beta][8]      |
...
[4]: https://github.com/openjdk/jdk26u/tags
[5]: https://github.com/openjdk/jdk27u/tags
...
For example, I'll demonstrate how I verify the OpenJDK Snap package installed on my system at the time of this writing. The `snap info` command shows that I installed OpenJDK version 26.0.2.1+1 with revision 2682, the revision for the *amd64* architecture:

```console
$ snap info openjdk
...
installed:          26.0.2.1+1            (2682) 289MB -
```
...
image-info:
  build-request-id: lp-117920029
  build-request-timestamp: '2026-08-18T23:23:38Z'
  build_url: https://launchpad.net/~jgneff/openjdk-snap/+snap/openjdk-candidate/+build/3250301
```

The `build_url` in the manifest is a link to the [page on Launchpad](https://launchpad.net/~jgneff/openjdk-snap/+snap/openjdk-candidate/+build/3250301) with the package's **Build status** and **Store status**. The store status shows that Launchpad uploaded revision 2682 to the Snap Store, which matches the revision installed on my system. The build status shows a link to the log file with the label [buildlog](https://launchpad.net/~jgneff/openjdk-snap/+snap/openjdk-candidate/+build/3250301/+files/buildlog_snap_ubuntu_focal_amd64_openjdk-candidate_BUILDING.txt.gz).

The end of the log file contains a line with the SHA512 checksum of the package just built, shown truncated below to fit on this page:

```
Snapping...
Snapped openjdk_26.0.2.1+1_amd64.snap
...
6ff8ebfdd08e...1f3626f8bc8f  openjdk_26.0.2.1+1_amd64.snap
Revoking proxy token...
[Tue Aug 18 23:54:04 2026]
```

The command below prints the checksum of the package installed on my system:

```console
$ sudo sha512sum /var/lib/snapd/snaps/openjdk_2682.snap
6ff8ebfdd08e...1f3626f8bc8f  /var/lib/snapd/snaps/openjdk_2682.snap
```
...
```console
$ openjdk
/var/snap/openjdk/2682/openjdk.env
```
...
export JAVA_HOME=/snap/openjdk/2682/jdk
export MANPATH=/snap/openjdk/2682/jdk/man:
...
```console
$ printenv | grep JAVA
JAVA_HOME=/snap/openjdk/2682/jdk
$ type java javac
...
See the [Snapcraft Documentation](https://ubuntu.com/docs/snapcraft/latest/) for more information about building Snap packages.
